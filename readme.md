...
#### 缺点：
 `目前暂不支持在词内渲染，未基于目前流行库canvas碰撞检测的做法，相比带来的好处就是更小的内存消耗，但不适合渲染大量数据的业务情况，基于dom绘制的模式下，很多函数依赖浏览器api，所以无法使用时间分片这种机制去优化处理过程，使用requestAnimateFrame或者requestIdeaCallback此类API时，css绘制任务权重会大于上述两种，所以时间分片模式下处理数据会变得极慢，目前索性就直接全部干成同步任务,数据量在100以下时没有很大的影响，当单词超出时或者区域渲染内容空闲空间太多时会做居中`
### 纯ts开发，无任何第三方依赖
### 基于函数式编程
### 扩展性强，可自定义追加动画和其他数据
### TS构建，丰富的测试用例
